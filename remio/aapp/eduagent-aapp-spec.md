...
> E10 以首行 `GROUNDED` / `MATERIAL` / `INSUFFICIENT` 标记显式上报覆盖度，升级到网络必须
> 显式发生；③ 来源分级标注：`[来源：章节>小节]`（知识库）、`📎 材料文件名`（会话材料）
...
| E11 | `attach_material` | SessionMaterial | `write_note` + `search_notes` | 材料文本/文件 | card（材料摘要） |

> `generate_animation`（动画分镜脚本）与 `search_video`（B 站视频）为可选增强端点，逻辑
> 与 E8/E9 一致，评委演示时可作为"资源类型 > 5"的加分展示。
> **E10 答疑知识来源为三级兜底**：课程知识库（GROUNDED）→ 会话学习材料（MATERIAL）→
> 网络（INSUFFICIENT 显式升级），任何一层切换都必须显式告知学生来源，禁止静默切换。
...
- **逻辑**：
  ...
  2. 知识库未命中且会话挂载了材料（E11）时，`search_material` 基于会话材料生成；
  3. `run_prompt(capabilities="none")` 用 DocAgent 人设 + 画像 + 上下文，生成中文讲义（主题概览→核心概念→学习步骤→常见误区→复习建议）；
  4. 文末附 `[来源：章节>小节]` 或 `📎 材料文件名`；知识库与材料均未命中时明确标注
...
### E10 · 答疑 `tutor_answer`（三级，禁止静默切换）

- **一级（知识库）**：`search_notes` + `read_note` 锚定 → `run_prompt(capabilities="none")`
  苏格拉底式解答；模型首行输出 `GROUNDED` / `INSUFFICIENT` 显式上报知识库覆盖度。
- **二级（会话学习材料）**：仅当一级 `INSUFFICIENT`、且该会话已通过 E11 挂载学习材料时触发：
  `search_material`（会话材料 note，带 `source=session-material` 元数据）→ 基于材料作答；
  回答前明确告知"课程知识库未充分覆盖此问题，以下基于你上传的材料"，并标注材料文件名。
- **三级（网络）**：仅当一、二级均未命中或用户传 `deep=true` 时触发：
  `web_search` → `web_get` 抓正文 → 基于抓取内容作答；回答前明确告知
  "课程知识库与已上传材料均未覆盖此问题，以下来自网络"。
- **来源分级**：`📚 课程知识库`（`[来源：章节>小节]`）、`📎 会话材料`（材料文件名）与
  `🌐 网络`（`[网络来源](URL)`，仅引用实际抓取过的页面）分栏展示；
  联网不可用时诚实降级为知识库/材料作答 + 覆盖度提示。
- **输出**：card（解答）+ text（分级来源）。

### E11 · 挂载学习材料 `attach_material`

- **触发**：学生上传自己的笔记/课件/讲义（md/txt/pdf/pptx 提取文本）。
- **逻辑**：`write_note` 把材料写入会话级 note（标题带会话前缀，元数据
  `source=session-material`、`session_id=xxx`）；`search_material` 即对这批 note 检索。
- **隔离**：材料仅属于对应会话，课程知识库 `search_notes` 不返回材料，材料检索不返回课程知识库。
- **输出**：card 展示已挂载文件名、块数、字符数，可删除。
- **后续**：E4 讲义 / E10 答疑在知识库未命中时自动基于材料生成并标注来源。
