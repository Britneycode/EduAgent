...
> 本仓库核心是一套"个性化多智能体学习系统"（LangGraph 多 Agent + RAG + 结构化课程知识库）。
> 本次把它按赛事要求的 "remio aApp / skill" 形态重新表达，并补齐参赛材料。
...
│   └── EDUAGENT_AAPP_STUDIO_操作清单.md ← aapp-studio 逐步操作清单
├── mcp/
...
- `backend/app/mcp_server.py` — 把 10 Agent 多智能体引擎封装为 MCP 工具集（14 个工具，零新增依赖，已自检通过）。

## 改造思路（差距分析结果）

| 比赛要求 | 原项目形态 | 改造动作 |
| --- | --- | --- |
| 作品形式 = remio aApp/skill | 多 Agent 引擎 | 新增《aApp 开发规格》+ manifest，把 10 个 Agent 重表达为语义端点 + remio 能力调用 |
| 可在 remio 正常运行 | 不依赖 remio | 规格完全按 remio「语义端点→对话为主→UI为辅→订阅自动化→发布」范式编写 |
| 如能在其他智能体产品正常运行更佳 | 引擎自闭环 | 新增 MCP 工具集（`backend/app/mcp_server.py`），跨产品可调 |
...
3. **提交材料**：按 `docs/01–06` 整理方案文档、Demo、演示视频与路演 PPT。
