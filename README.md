...
| 形态 | 位置 | 说明 |
|---|---|---|
| remio aApp（主力作品） | 已上架市场（id `eduagent-pro`），规格 `remio/`，开发副本在 remio 客户端 aapps-dev | 10 Agent 重表达为 18 个端点（核心 E1–E11 + 判题/配图/复习/动画/视频扩展），运行在 remio 睿妙：知识库锚定答疑、联网拓展阅读、三级可信来源标注 |
...
- **多课程知识库**：计算机网络（13 章全栈资源：总纲/讲解/实验/题库/代码案例/媒体/事实卡/知识图谱，默认课程）、算法设计与分析（10 章 + 习题/代码/实验/媒体配套），多课程自动发现与切换
