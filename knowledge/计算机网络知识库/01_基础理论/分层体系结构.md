...
keywords: ["OSI", "TCP/IP", "五层模型", "封装"]
aliases: ["分层体系结构"]
summary: "说明 OSI、TCP/IP 与五层教学模型的层次职责、封装关系和学习价值。"
learning_goals: ["理解核心概念", "解释典型流程", "识别常见误解"]
prerequisites: []
related_docs: []
related_concepts: ["OSI", "TCP/IP", "五层模型", "封装"]
graph_nodes: []
graph_edges: []
...
rag:
  ...
  embedding_hints: ["OSI", "TCP/IP", "五层模型", "封装", "分层体系结构"]
quality:
  factual_risk: "medium"
...
## 1. 学习目标
- 理解分层体系结构的基本定义、作用范围和所在层次。
- 能够用分层模型解释分层体系结构与相邻知识点的关系。
- 能够基于事实卡、实验或题目验证关键结论。

## 2. 核心概念
分层体系结构把复杂网络通信拆成相对独立的层，每层向上提供服务，向下使用服务。

## 3. 知识讲解
说明 OSI、TCP/IP 与五层教学模型的层次职责、封装关系和学习价值。 学习时应先判断该主题属于哪一层，再分析它处理的数据单位、关键字段、典型流程和失效场景。对于 LLM/RAG 使用，本节优先提供稳定表述，详细推导、标准引用与实验截图在后续审核版本中补充。
...
## 6. 导师 Agent 教学提示
- 学生常见困惑：记不住 OSI 七层顺序，或混淆各层职责。
- 诊断建议：让学生尝试从应用层向下逐层描述"发送一封邮件"的过程。
- 个性化策略：用"寄信"类比——应用层写信、运输层装信封、网络层写地址、链路层装邮包、物理层运输。
