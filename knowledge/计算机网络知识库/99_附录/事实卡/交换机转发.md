...
keywords: ["交换机转发", "事实卡"]
aliases: ["交换机转发"]
summary: "交换机根据源 MAC 学习地址表，根据目的 MAC 决定转发、过滤或泛洪。"
learning_goals: ["理解核心概念", "解释典型流程", "识别常见误解"]
prerequisites: []
related_docs: []
related_concepts: ["交换机转发", "事实卡"]
graph_nodes: []
graph_edges: []
...
rag:
  ...
  embedding_hints: ["交换机转发", "事实卡", "交换机转发"]
quality:
  factual_risk: "high"
...
## 标准表述
交换机根据源 MAC 学习地址表，根据目的 MAC 决定转发、过滤或泛洪。

## 适用场景
局域网转发题和交换机实验。

## 常见误解
误以为普通二层交换机主要根据 IP 地址转发。

## 可验证依据
- IEEE 802.1D - MAC Bridging 标准。
- 谢希仁《计算机网络》（第8版）第3.4节。
- 当前状态：`verified: true`（转发机制与标准一致）。
