...
keywords: ["IP协议", "事实卡"]
aliases: ["IP协议"]
summary: "IP 提供无连接、尽力而为的数据报传递服务，不保证可靠、按序或不重复交付。"
learning_goals: ["理解核心概念", "解释典型流程", "识别常见误解"]
prerequisites: []
related_docs: []
related_concepts: ["IP协议", "事实卡"]
graph_nodes: []
graph_edges: []
...
## 标准表述
IP 提供无连接、尽力而为的数据报传递服务，不保证可靠、按序或不重复交付。

## 适用场景
网络层基础讲解、IP 与 TCP 职责比较。

## 常见误解
误以为 IP 本身提供端到端可靠传输。

## 可验证依据
- RFC 791 (IPv4) - IP 协议正式规范。
- 谢希仁《计算机网络》（第8版）第4.2节。
- 当前状态：`verified: true`（基本服务模型与 RFC 一致）。
