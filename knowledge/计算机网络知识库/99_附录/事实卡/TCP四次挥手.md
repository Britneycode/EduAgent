...
keywords: ["TCP四次挥手", "事实卡"]
aliases: ["TCP四次挥手"]
summary: "TCP 连接释放常用四次挥手描述：一方发送 FIN，另一方 ACK；另一方向关闭时再发送 FIN，原发送方 ACK。全双工连接的两个方向可独立关闭。"
learning_goals: ["理解核心概念", "解释典型流程", "识别常见误解"]
prerequisites: []
related_docs: []
related_concepts: ["TCP四次挥手", "事实卡"]
graph_nodes: []
graph_edges: []
...
rag:
  ...
  embedding_hints: ["TCP四次挥手", "事实卡", "TCP四次挥手"]
quality:
  factual_risk: "high"
...
## 标准表述
TCP 连接释放常用四次挥手描述：一方发送 FIN，另一方 ACK；另一方向关闭时再发送 FIN，原发送方 ACK。全双工连接的两个方向可独立关闭。

## 适用场景
讲解连接释放、TIME_WAIT、抓包综合题。

## 常见误解
误以为四次挥手在所有抓包中一定表现为四个独立报文段。

## 可验证依据
- RFC 9293 (TCP) Section 3.5 - 定义连接终止过程。
- 谢希仁《计算机网络》（第8版）第5.6节 - 四次挥手教学讲解。
