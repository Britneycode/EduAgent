...
keywords: ["数据链路层", "帧", "差错检测", "MAC"]
aliases: ["数据链路层概述"]
summary: "概述成帧、差错检测、介质访问控制和局域网转发。"
learning_goals: ["理解核心概念", "解释典型流程", "识别常见误解"]
prerequisites: []
related_docs: []
related_concepts: ["数据链路层", "帧", "差错检测", "MAC"]
graph_nodes: []
graph_edges: []
...
rag:
  ...
  embedding_hints: ["数据链路层", "帧", "差错检测", "MAC", "数据链路层概述"]
quality:
  factual_risk: "medium"
...
## 1. 学习目标
- 理解数据链路层概述的基本定义、作用范围和所在层次。
- 能够用分层模型解释数据链路层概述与相邻知识点的关系。
- 能够基于事实卡、实验或题目验证关键结论。
...
## 3. 知识讲解
概述成帧、差错检测、介质访问控制和局域网转发。 学习时应先判断该主题属于哪一层，再分析它处理的数据单位、关键字段、典型流程和失效场景。对于 LLM/RAG 使用，本节优先提供稳定表述，详细推导、标准引用与实验截图在后续审核版本中补充。
...
## 导师 Agent 教学提示
- 学生常见困惑：分不清数据链路层和网络层的职责边界。
- 诊断建议：问"MAC 地址和 IP 地址分别在哪一层？它们的作用范围有什么不同？"
- 个性化策略：用"门牌号 vs 邮政编码"类比——MAC 像小区内的门牌号（本链路），IP 像邮政编码（跨网络）。

## 6. 例题与解析
**例题：** 请说明数据链路层概述解决的核心问题。
