...
keywords: ["HTTP抓包实验", "实验"]
aliases: ["HTTP抓包实验"]
summary: "通过 Wireshark 或浏览器开发者工具观察 HTTP 请求和响应报文。"
learning_goals: ["理解核心概念", "解释典型流程", "识别常见误解"]
prerequisites: []
related_docs: []
related_concepts: ["HTTP抓包实验", "实验"]
graph_nodes: []
graph_edges: []
...
rag:
  ...
  embedding_hints: ["HTTP抓包实验", "实验", "HTTP抓包实验"]
quality:
  factual_risk: "medium"
...
## 1. 实验目标
通过 Wireshark 或浏览器开发者工具观察 HTTP 请求和响应报文。
...
## 3. 操作步骤
1. 启动 Wireshark，设置过滤器 `http`。
2. 用浏览器访问一个 HTTP 网站（非 HTTPS）。
3. 观察 TCP 三次握手后 HTTP GET 请求和服务器响应。
4. 记录请求方法、URI、Host 首部、状态码和 Content-Type。
5. 对比 HTTPS 流量（TLS 加密后应用层不可见）。
