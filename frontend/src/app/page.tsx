...
const FEATURES = [
  { icon: "🧠", title: "8 Agent 协同", desc: "Router、Planner、Tutor、Doc、Quiz、Code、Media、Reading 八个 AI Agent 分工协作" },
  { icon: "📝", title: "智能出题", desc: "选择题、判断题、简答题一键生成，逐题交互，答完即解析，AI 实时讨论" },
  { icon: "🖼️", title: "PPT 自动生成", desc: "AI 提炼知识要点，调用图片模型生成专业蓝白商务风 PPT 演示图片" },
  { icon: "📊", title: "学习画像", desc: "8 维度雷达图精准刻画学习状态，动态更新认知风格、薄弱点和兴趣方向" },
  { icon: "🔄", title: "错题复习", desc: "按艾宾浩斯遗忘曲线自动排期，错题回归提醒，知识盲点逐个击破" },
  { icon: "📚", title: "知识中枢", desc: "课程文档向量化存储，RAG 混合检索，DAG 知识图谱可视化" },
  { icon: "🎯", title: "学习路径", desc: "基于前置知识的个性化学习路径，自动推荐下一步学习方向" },
  { icon: "📈", title: "学习报告", desc: "每周学习统计、趋势图表、知识点掌握度一览，数据驱动进步" },
];
    ...
    <div className="min-h-screen bg-[var(--color-parchment)]">
      ...
      <header className="border-b border-[var(--color-warm-gray-200)] bg-[var(--color-ivory)]/95 backdrop-blur-sm sticky top-0 z-50">
        <div className="mx-auto flex max-w-6xl items-center justify-between px-6 py-4">
          ...
          <div className="flex items-center gap-4">
            ...
            <Link href="/register" className="rounded-xl bg-[var(--color-terracotta)] px-5 py-2 text-sm text-white hover:bg-[var(--color-terracotta-hover)]">免费注册</Link>
          </div>
        </div>
      </header>
      ...
      <section className="relative overflow-hidden px-6 py-20 md:py-32">
        <div className="mx-auto max-w-4xl text-center">
          <div className="mb-6 inline-flex items-center gap-2 rounded-full bg-[var(--color-ivory)] px-4 py-1.5 text-xs text-[var(--color-warm-gray-600)] ring-1 ring-[var(--color-warm-gray-200)]">
            🏆 基于 LangGraph 多智能体架构 · 讯飞星火 + 阿里百炼双引擎
          </div>
          ...
          <div className="flex flex-col items-center gap-4 sm:flex-row sm:justify-center">
            <Link
              href="/register"
              className="rounded-2xl bg-[var(--color-terracotta)] px-10 py-4 text-lg font-medium text-white shadow-lg shadow-[var(--color-terracotta)]/25 transition-all hover:bg-[var(--color-terracotta-hover)] hover:shadow-xl"
            >
              免费开始使用 →
            </Link>
            <Link
      ...
      <section className="px-6 pb-20">
        <div className="mx-auto max-w-6xl">
          ...
          <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-4">
            {FEATURES.map((f) => (
              <div key={f.title} className="rounded-2xl bg-[var(--color-ivory)] p-6 ring-1 ring-[var(--color-warm-gray-200)] hover:ring-[var(--color-terracotta)]/30 transition-all">
      ...
      <section className="border-t border-[var(--color-warm-gray-200)] bg-[var(--color-ivory)] px-6 py-20">
        <div className="mx-auto max-w-4xl text-center">
          ...
          <div className="grid gap-8 md:grid-cols-3">
            {[
              { step: "1", icon: "✍️", title: "完善画像", desc: "花 2 分钟告诉 AI 你的专业、目标和学习偏好" },
              { step: "2", icon: "💬", title: "说出需求", desc: "自然语言描述——\"帮我梳理机器学习学习路径\"\"出几道题练练手\"" },
              { step: "3", icon: "🚀", title: "开始学习", desc: "AI 为你生成个性化学习资料，练习、复习、追踪全面提升" },
            ].map((s) => (
              <div key={s.step} className="rounded-2xl bg-[var(--color-parchment)] p-6 ring-1 ring-[var(--color-warm-gray-200)]">
                <div className="mx-auto mb-4 flex h-14 w-14 items-center justify-center rounded-full bg-[var(--color-terracotta)]/10 text-2xl">
                  {s.icon}
                </div>
      ...
      <section className="px-6 py-20 text-center">
        <div className="mx-auto max-w-2xl">
          ...
          <Link
            href="/register"
            className="inline-block rounded-2xl bg-[var(--color-terracotta)] px-12 py-4 text-lg font-medium text-white shadow-lg shadow-[var(--color-terracotta)]/25 transition-all hover:bg-[var(--color-terracotta-hover)]"
          >
            免费注册 →
          </Link>
        </div>
      ...
      <footer className="border-t border-[var(--color-warm-gray-200)] bg-[var(--color-ivory)] px-6 py-8 text-center">
        <p className="text-xs text-[var(--color-warm-gray-400)]">
          © 2025 EduAgent · 基于 LangGraph + 讯飞星火 + 阿里百炼 · 高等教育 AI 学习平台
        </p>
      </footer>
    </div>
  );
