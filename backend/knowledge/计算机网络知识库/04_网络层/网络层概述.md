...
keywords: ["网络层", "路由", "转发", "IP"]
aliases: ["网络层概述"]
summary: "概述网络层寻址、路由、转发和互联网数据报服务。"
learning_goals: ["理解核心概念", "解释典型流程", "识别常见误解"]
prerequisites: []
related_docs: []
related_concepts: ["网络层", "路由", "转发", "IP"]
graph_nodes: []
graph_edges: []
...
rag:
  ...
  embedding_hints: ["网络层", "路由", "转发", "IP", "网络层概述"]
quality:
  factual_risk: "medium"
...
## 1. 学习目标
- 理解网络层概述的基本定义、作用范围和所在层次。
- 能够用分层模型解释网络层概述与相邻知识点的关系。
- 能够基于事实卡、实验或题目验证关键结论。

## 2. 核心概念
网络层负责把分组跨越多个网络送到目的主机。

## 3. 知识讲解
概述网络层寻址、路由、转发和互联网数据报服务。 学习时应先判断该主题属于哪一层，再分析它处理的数据单位、关键字段、典型流程和失效场景。对于 LLM/RAG 使用，本节优先提供稳定表述，详细推导、标准引用与实验截图在后续审核版本中补充。
...
## 导师 Agent 教学提示
- 学生常见困惑：混淆路由和转发。
- 诊断建议：问"路由表是怎么生成的？转发表是怎么使用的？"
- 个性化策略：路由像"规划导航路线"（控制平面），转发像"在每个路口按导航转弯"（数据平面）。
