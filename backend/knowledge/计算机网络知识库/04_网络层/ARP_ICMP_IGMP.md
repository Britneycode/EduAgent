...
keywords: ["ARP", "ICMP", "IGMP", "网络控制"]
aliases: ["ARP_ICMP_IGMP"]
summary: "概述 ARP、ICMP、IGMP 的用途和与 IP 的关系。"
learning_goals: ["理解核心概念", "解释典型流程", "识别常见误解"]
prerequisites: []
related_docs: []
related_concepts: ["ARP", "ICMP", "IGMP", "网络控制"]
graph_nodes: []
graph_edges: []
...
rag:
  ...
  embedding_hints: ["ARP", "ICMP", "IGMP", "网络控制", "ARP_ICMP_IGMP"]
quality:
  factual_risk: "medium"
...
## 1. 学习目标
- 理解ARP_ICMP_IGMP的基本定义、作用范围和所在层次。
- 能够用分层模型解释ARP_ICMP_IGMP与相邻知识点的关系。
- 能够基于事实卡、实验或题目验证关键结论。

## 2. 核心概念
ARP、ICMP、IGMP 分别服务地址解析、控制报文和组播成员管理。

## 3. 知识讲解
概述 ARP、ICMP、IGMP 的用途和与 IP 的关系。 学习时应先判断该主题属于哪一层，再分析它处理的数据单位、关键字段、典型流程和失效场景。对于 LLM/RAG 使用，本节优先提供稳定表述，详细推导、标准引用与实验截图在后续审核版本中补充。
...
## 导师 Agent 教学提示
- 学生常见困惑：混淆 ARP 和 DNS 的功能。
- 诊断建议：问"ARP 解析的是什么地址到什么地址？DNS 呢？"
- 个性化策略：ARP 是"问邻居的门牌号"（IP→MAC，本链路），DNS 是"查电话簿"（域名→IP，全球）。

## 6. 例题与解析
**例题：** 请说明ARP_ICMP_IGMP解决的核心问题。
