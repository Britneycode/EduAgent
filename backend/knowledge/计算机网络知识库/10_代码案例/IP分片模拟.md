...
keywords: ["IP分片模拟", "代码案例"]
aliases: ["IP分片模拟"]
summary: "用伪代码解释 MTU 约束下的 IPv4 分片思想。"
learning_goals: ["理解核心概念", "解释典型流程", "识别常见误解"]
prerequisites: []
related_docs: []
related_concepts: ["IP分片模拟", "代码案例"]
graph_nodes: []
graph_edges: []
...
rag:
  ...
  embedding_hints: ["IP分片模拟", "代码案例", "IP分片模拟"]
quality:
  factual_risk: "medium"
...
## 1. 案例目标
用伪代码解释 MTU 约束下的 IPv4 分片思想。

## 2. 示例代码 / 伪代码
```text
每片数据最大长度 = floor((mtu - header_length) / 8) * 8
循环切片，设置 identification、fragment_offset 和 MF 标志
最后一片 MF = 0
```
